...
head'' vec | length vec > 0 = Just (vec ! 0)
           | otherwise      = Nothing
\end{code}
...
\begin{code}
{-@ unsafeLookup :: i:Nat -> {v:Vector a | vlen v > i} -> a @-}
...
`vec ! i` is unsafe because `i` might equal `sz` and be out-of-bounds.
...
Because the loop ends when `go` is called with `i == sz`,
triggering the `otherwise` branch.
...
body :: {v:Nat | v < vlen vec} -> acc:Nat -> {v:Nat | v >= acc}
